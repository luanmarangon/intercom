...
<body>
    ...
    <div class="container">
        ...
        $endereco = $objDAO->consultarMoradorEndereco($_GET['id']);
        include("classes/dao/MoradoresDAO.class.php");
        $objDAO = new MoradoresDAO();
        $consulta = $objDAO->consultarCodigoMoradores($endereco['moradores_id']);
        ...
        <form action="classes/controller/RamaisController.php" method="POST">
            <input type="hidden" name="morador" value="<?php echo $endereco['moradores_id']; ?>" />
            <h3>Cadastro de Ramal do Morador: <strong> <?php echo $consulta['nome']; ?> </strong> </h3>
            <h4>Endereço: <strong> <?php echo $endereco['logradouro'] . ', ' . $endereco['numero'] . ' - ' . $condominio['nome']; ?> </strong></h4>
            <input type="hidden" name="grupo" value="<?php echo $endereco['grupo']; ?>" />
            ...
            <div class="row">
                <div class="col-sm-4">
                    <input type="submit" class="btn btn-primary btn-lg" value="Cadastrar" name="btnCadastrarRamal">
